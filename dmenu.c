...
/* enums */
enum { SchemeNorm, SchemeSel, SchemePrompt, SchemeInput, SchemeOut, SchemeLast }; /* color schemes */
...
drawmenu(void)
{
	...
	if (lines > 0) {
		...
		for (item = curr; item != next; item = item->right, i++)
			drawitem(item, 0, y += bh, mw - x + promptw);
...
usage(void)
{
	fputs("usage: dmenu [-bfivP] [-l lines] [-lh line height] [-p prompt] [-fn font] [-m monitor]\n"
